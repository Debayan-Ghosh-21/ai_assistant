...
**Internal**: `dyslexxy.domain.notebook` (Source, Note, SourceInsight), `dyslexxy.utils.chunking` (chunk_text, detect_content_type), `dyslexxy.utils.embedding` (generate_embedding, generate_embeddings), `dyslexxy.database.repository` (repo_query, repo_insert)
...
```python
@command("process_source", app="dyslexxy", retry={
    "max_attempts": 5,
