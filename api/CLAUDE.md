...
- **Error handling**: Custom exception hierarchy (`dyslexxy.exceptions`) with global FastAPI exception handlers mapping to HTTP status codes (see Error Handling section below). LangGraph nodes use `classify_error()` to convert raw LLM provider errors into typed exceptions with user-friendly messages.
...
- `dyslexxy.graphs`: chat, ask, source_chat, source, transformation graphs
- `dyslexxy.database`: SurrealDB repository functions (repo_query, repo_create, repo_upsert)
- `dyslexxy.domain`: Notebook, Source, Note, SourceInsight models
- `dyslexxy.ai.provision`: provision_langchain_model() factory
...
FastAPI exception handlers map custom exception types from `dyslexxy.exceptions` to HTTP status codes. All error responses include CORS headers.
...
| `DyslexxyError` (base) | 500 | Any other application error |

### Error Classification (`dyslexxy.utils.error_classifier`)
...
- Requires `DYSLEXXY_ENCRYPTION_KEY` to be set for storing credentials

### Domain Model: `Credential` (`dyslexxy/domain/credential.py`)
...
### Integration with Key Provider (`dyslexxy/ai/key_provider.py`)
...
- Default password: `dyslexxy-change-me` (set `DYSLEXXY_PASSWORD` in production)
- Docker secrets support via `DYSLEXXY_PASSWORD_FILE`

### Connection Testing (`dyslexxy/ai/connection_tester.py`)
