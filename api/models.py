...
class ApiKeyStatusResponse(BaseModel):
    ...
    encryption_configured: bool = Field(
        ...,
        description="Whether DYSLEXXY_ENCRYPTION_KEY is set (required to store keys in database)",
    )
...
class GenerateQuizRequest(BaseModel):
    ...
    source_id: str = Field(..., description="Source ID to generate quiz from")
    num_questions: int = Field(
        5, ge=1, le=20, description="Number of questions to generate"
    )
    difficulty: Literal["easy", "medium", "hard"] = Field(
        "medium", description="Quiz difficulty level"
    )
    model_override: Optional[str] = Field(
        None, description="Optional model ID override"
    )


class SubmitAnswersRequest(BaseModel):
    answers: Dict[int, str] = Field(
        ..., description="Map of question index to selected answer (A/B/C/D)"
    )
