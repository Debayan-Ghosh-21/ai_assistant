export const zhCN = {
  ...
  advanced: {
    ...
    updateAvailableDesc: "Dyslexxy 的新版本可用。",
    upToDate: "已是最新",
    unknown: "未知",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "配置用于 Dyslexxy 不同用途的默认模型",
    missingRequiredModels: "缺少必需的模型：{models}。如果没有这些模型，Dyslexxy 可能无法正常运行。",
  ...
  apiKeys: {
    ...
    description: "将 API 密钥安全地存储在数据库中，以在 Dyslexxy 中启用 AI 服务商。",
    encryptionRequired: "未配置加密密钥",
    encryptionRequiredDescription: "请将 DYSLEXXY_ENCRYPTION_KEY 环境变量设置为任意密钥字符串，以启用将 API 密钥存储到数据库。",
    configured: "已配置",
    notConfigured: "未配置",
  ...
  setupBanner: {
    encryptionRequired: "未配置加密密钥",
    encryptionRequiredDescription: "请设置 DYSLEXXY_ENCRYPTION_KEY 环境变量以启用安全凭据存储。",
