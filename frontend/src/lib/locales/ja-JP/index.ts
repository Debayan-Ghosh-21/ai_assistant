export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "Dyslexxy APIサーバーに到達できませんでした",
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはDyslexxyがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Dyslexxyの新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "Dyslexxy全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {models}。これらがないとDyslexxyが正しく機能しない可能性があります。",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、DyslexxyでAIプロバイダーを有効にします。",
    encryptionRequired: "暗号化キーが設定されていません",
    encryptionRequiredDescription: "DYSLEXXY_ENCRYPTION_KEY 環境変数に任意の秘密文字列を設定して、データベースへのAPIキーの保存を有効にしてください。",
    configured: "設定済み",
    notConfigured: "未設定",
  ...
  setupBanner: {
    encryptionRequired: "暗号化キーが設定されていません",
    encryptionRequiredDescription: "DYSLEXXY_ENCRYPTION_KEY 環境変数を設定して、安全な認証情報の保存を有効にしてください。",
