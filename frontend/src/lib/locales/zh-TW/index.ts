export const zhTW = {
  ...
  models: {
    ...
    defaultAssignmentsDesc: "設定用於 Dyslexxy 不同用途的預設模型",
    missingRequiredModels: "缺少必需的模型：{models}。如果没有這些模型，Dyslexxy 可能無法正常運行。",
  ...
  apiKeys: {
    ...
    description: "將 API 金鑰安全地儲存在資料庫中，以在 Dyslexxy 中啟用 AI 服務商。",
    encryptionRequired: "未設定加密金鑰",
    encryptionRequiredDescription: "請將 DYSLEXXY_ENCRYPTION_KEY 環境變數設定為任意密鑰字串，以啟用將 API 金鑰儲存至資料庫。",
    configured: "已設定",
    notConfigured: "未設定",
  ...
  setupBanner: {
    encryptionRequired: "未設定加密金鑰",
    encryptionRequiredDescription: "請設定 DYSLEXXY_ENCRYPTION_KEY 環境變數以啟用安全憑據儲存。",
