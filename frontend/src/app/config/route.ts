...
export async function GET(request: NextRequest) {
  ...
  // Priority 2 & 3: Default to relative paths
  // This forces the frontend to use the Next.js proxy, avoiding all CORS/port issues
  console.log('[runtime-config] Using relative path for API_URL (proxy mode)')
  return NextResponse.json({
    apiUrl: '',
  })
}
