...
This document outlines security practices for Dyslexxy development. It is informed by real vulnerabilities discovered through coordinated disclosure with [CERT-EU](https://cert.europa.eu) and should be treated as mandatory reading for all contributors.

## Reporting Vulnerabilities

If you discover a security vulnerability, **do not open a public GitHub issue**. Instead:

1. Use [GitHub Security Advisories](https://github.com/lfnovo/dyslexxy/security/advisories/new) to report privately
...
Dyslexxy currently uses simple password-based middleware (`PasswordAuthMiddleware`). This is suitable for single-user self-hosted deployments but should be hardened for production:

- Change the default password (`DYSLEXXY_PASSWORD`)
- Change the default encryption key (`DYSLEXXY_ENCRYPTION_KEY`)
- Consider deploying behind a reverse proxy with proper authentication (OAuth, OIDC)

### CORS

The default CORS configuration allows all origins (`allow_origins=["*"]`). This is tracked for improvement in [#730](https://github.com/lfnovo/dyslexxy/issues/730). For production deployments, restrict origins to only the frontend URL.
...
`DYSLEXXY_ENCRYPTION_KEY` is used to encrypt API keys stored in SurrealDB. In production:

- Set a strong, unique key (do not use the default)
- Use Docker secrets via `DYSLEXXY_ENCRYPTION_KEY_FILE` when possible
...
|---------|--------------|----------|----------|
| <= 1.8.2 | SurrealDB injection via `order_by` parameter | High (8.7) | [GHSA-5wj9-f8q5-8f9c](https://github.com/lfnovo/dyslexxy/security/advisories/GHSA-5wj9-f8q5-8f9c) |
| <= 1.8.3 | RCE via Jinja2 SSTI in transformations | Critical (9.2) | [GHSA-f35w-wx37-26q7](https://github.com/lfnovo/dyslexxy/security/advisories/GHSA-f35w-wx37-26q7) |
| <= 1.8.3 | Arbitrary file write via path traversal | High (7.0) | [GHSA-x4q2-89g5-594v](https://github.com/lfnovo/dyslexxy/security/advisories/GHSA-x4q2-89g5-594v) |
| <= 1.8.3 | Arbitrary file read via LFI | High (8.2) | [GHSA-842v-h4cj-r646](https://github.com/lfnovo/dyslexxy/security/advisories/GHSA-842v-h4cj-r646) |
