# Dyslexxy Architecture

## High-Level Overview

Dyslexxy follows a three-tier architecture with clear separation of concerns:
...
LangGraph is a state machine library that orchestrates multi-step AI workflows. Dyslexxy uses five core workflows:

### 1. **Source Processing Workflow** (`dyslexxy/graphs/source.py`)
...
### 2. **Chat Workflow** (`dyslexxy/graphs/chat.py`)
...
### 3. **Ask Workflow** (`dyslexxy/graphs/ask.py`)
...
### 4. **Transformation Workflow** (`dyslexxy/graphs/transformation.py`)
...
### 5. **Prompt Workflow** (`dyslexxy/graphs/prompt.py`)
...
Located in `dyslexxy/ai/models.py`, ModelManager handles:
...
**Domain Objects** (`dyslexxy/domain/`):
...
**Repository Pattern**:
- Database access layer (`dyslexxy/database/repository.py`)
...
1. Create `dyslexxy/graphs/workflow_name.py`
...
1. Create model in `dyslexxy/domain/model_name.py`
