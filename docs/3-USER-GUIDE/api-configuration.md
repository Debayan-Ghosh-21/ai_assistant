...
> **Credential System**: Dyslexxy uses encrypted credentials stored in the database. Each credential connects to a provider and allows you to discover, register, and test models.

---

## Overview

Dyslexxy manages AI provider access through a **credential-based system**:
...
Add `DYSLEXXY_ENCRYPTION_KEY` to your docker-compose.yml:
...
3. The models are now available throughout Dyslexxy
...
- Keep `DYSLEXXY_ENCRYPTION_KEY` — it's still required
...
|---------|---------------|---------------------------|
| Password | `dyslexxy-change-me` | Set `DYSLEXXY_PASSWORD` |
| Encryption Key | None (must be set) | Set `DYSLEXXY_ENCRYPTION_KEY` to any secret string |
...
| Error on save | Encryption key not set | Set `DYSLEXXY_ENCRYPTION_KEY` in docker-compose.yml |
