...
Use any server that implements the OpenAI API format with Dyslexxy. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Dyslexxy can connect to any server using this format.
...
When Dyslexxy runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
```bash
docker exec -it dyslexxy curl http://host.docker.internal:1234/v1/models
```
...
4. Update model name in Dyslexxy
```
