...
Deploy Dyslexxy behind nginx, Caddy, Traefik, or other reverse proxies with custom domains and HTTPS.

---

## Simplified Setup (v1.1+)

Starting with v1.1, Dyslexxy uses Next.js rewrites to simplify configuration. **You only need to proxy to one port** - Next.js handles internal API routing automatically.
...
# See full base setup: https://github.com/lfnovo/dyslexxy/blob/main/docker-compose.yml
services:
  dyslexxy:
...
> **Note:** This example only shows the dyslexxy and nginx services. You also need a `surrealdb` service. See the [full base docker-compose.yml](https://github.com/lfnovo/dyslexxy/blob/main/docker-compose.yml) for the complete setup.
...
services:
  dyslexxy:
    image: lfnovo/dyslexxy:v1-latest
    pull_policy: always
    container_name: dyslexxy
    environment:
      - API_URL=https://notebook.example.com
      - DYSLEXXY_ENCRYPTION_KEY=${DYSLEXXY_ENCRYPTION_KEY}
      - DYSLEXXY_PASSWORD=${DYSLEXXY_PASSWORD}
    volumes:
...
location /api/ {
    proxy_pass http://dyslexxy:5055/api/;
    proxy_http_version 1.1;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
Accessing Dyslexxy from a different machine on your network:

**Step 1: Get your server IP**
```bash
# On the server running Dyslexxy:
hostname -I
# or
ifconfig | grep "inet "
...
services:
  frontend:
    image: lfnovo/dyslexxy_frontend:v1-latest
    pull_policy: always
    environment:
      - API_URL=https://notebook.example.com
    ports:
  ...
  api:
    image: lfnovo/dyslexxy_api:v1-latest
    pull_policy: always
    environment:
      - DYSLEXXY_ENCRYPTION_KEY=${DYSLEXXY_ENCRYPTION_KEY}
    ports:
...
1. **Check API_URL is set**:
   ```bash
   docker exec dyslexxy env | grep API_URL
   ```
...
2. Check container logs: `docker logs dyslexxy`
...
**Cause:** Your reverse proxy has a default timeout (often 30s) that's shorter than Dyslexxy's operations.
...
**Version Requirement:**
- **Dyslexxy v1.3.2+** is required for file uploads >10MB
...
**Note:** Dyslexxy's API includes CORS headers in error responses, but this only works for errors that reach the application. Proxy-level errors (like 413 from nginx) need to be configured at the proxy level.
...
3. **Reverse proxy not forwarding `/api/*` correctly**:
   ...
   location /api/ {
       proxy_pass http://dyslexxy:5055/api/;  # Note the trailing slash!
   }
   ```
...
This happens when:
- You have set `DYSLEXXY_PASSWORD` for authentication
...
9. **Monitor logs** regularly: `docker logs dyslexxy`
...
If you're running Dyslexxy **version 1.0.x or earlier**, you may need to use the legacy two-port configuration where you explicitly route `/api/*` to port 5055.

**Check your version:**
```bash
docker exec dyslexxy cat /app/package.json | grep version
```
