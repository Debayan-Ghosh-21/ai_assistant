...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with Dyslexxy, including different deployment scenarios and network configurations.
...
When both Dyslexxy and Ollama run directly on your machine:
...
### Scenario 2: Dyslexxy in Docker, Ollama on Host

When Dyslexxy runs in Docker but Ollama runs on your host machine:
...
When both Dyslexxy and Ollama run in the same Docker Compose stack:
...
# https://github.com/lfnovo/dyslexxy/blob/main/docker-compose.yml
services:
  dyslexxy:
    image: lfnovo/dyslexxy:v1-latest
    pull_policy: always
...
This is the most common cause of "Failed to send message" errors. Dyslexxy requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in Dyslexxy**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in Dyslexxy, not just `model`.

**Step 3: Configure in Dyslexxy**
...
**1. "Ollama unavailable" in Dyslexxy**
...
```bash
# If Dyslexxy runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When Dyslexxy runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside Dyslexxy container, test Ollama:**
```bash
# Get into container
docker exec -it dyslexxy bash
...
1. **Model name mismatch**: The model name in Dyslexxy doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Settings → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update Dyslexxy's defaults
4. **Model record deleted**: The model was removed from Dyslexxy but is still set as default
...
# Compare with what's configured in Dyslexxy
...
docker run --network host lfnovo/dyslexxy:v1-latest  # for quick testing only
```
...
# Dyslexxy configuration (configure via Settings → API Keys instead)
...
- [Dyslexxy Discord](https://discord.gg/37XJPXfz2w) - Integration help
...
This comprehensive guide should help you successfully deploy and optimize Ollama with Dyslexxy. Start with the Quick Start section and refer to specific scenarios as needed.