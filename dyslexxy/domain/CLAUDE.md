...
- **Credential**: Individual credential records for API keys and provider configuration
  ...
  - **Encryption integration**: Uses `encrypt_value()`/`decrypt_value()` from `dyslexxy.utils.encryption`
    - Keys encrypted with Fernet before database storage
    - Requires `DYSLEXXY_ENCRYPTION_KEY` environment variable (warns if not set)
  - **Key methods**:
...
- `dyslexxy.database.repository`: CRUD and relationship functions
- `dyslexxy.ai.models`: ModelManager for embeddings
