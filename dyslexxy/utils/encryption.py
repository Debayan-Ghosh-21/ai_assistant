...
DYSLEXXY_ENCRYPTION_KEY accepts **any string**. A Fernet key is derived
from it via SHA-256, so users can set a simple passphrase like
``DYSLEXXY_ENCRYPTION_KEY=my-secret`` and it will work.
...
def get_secret_from_env(var_name: str) -> Optional[str]:
    ...
    Args:
        var_name: Base name of the environment variable (e.g., "DYSLEXXY_ENCRYPTION_KEY")
...
def _get_or_create_encryption_key() -> str:
    ...
    Priority:
    1. DYSLEXXY_ENCRYPTION_KEY_FILE (Docker secrets)
    2. DYSLEXXY_ENCRYPTION_KEY (environment variable)

    For production deployments, you MUST set DYSLEXXY_ENCRYPTION_KEY explicitly!
    ...
    key = get_secret_from_env("DYSLEXXY_ENCRYPTION_KEY")
    if key:
        return key

    raise ValueError(
        "DYSLEXXY_ENCRYPTION_KEY is not set. "
    ...
    try:
        ...
        if looks_like_fernet_token(value):
            ...
            raise ValueError(
                ...
                "Check DYSLEXXY_ENCRYPTION_KEY configuration."
            )
