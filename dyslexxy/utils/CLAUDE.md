...
- **DYSLEXXY_CHUNK_SIZE**: Maximum chunk size in tokens (default: 400)
  - Minimum: 100 tokens
...
- **DYSLEXXY_CHUNK_OVERLAP**: Overlap between chunks in tokens (default: 15% of CHUNK_SIZE)
...
```bash
export DYSLEXXY_CHUNK_SIZE=1500
export DYSLEXXY_CHUNK_OVERLAP=150
```
...
- **CHUNK_SIZE**: Configurable via `DYSLEXXY_CHUNK_SIZE` env var (default: 400)
- **CHUNK_OVERLAP**: Configurable via `DYSLEXXY_CHUNK_OVERLAP` env var (default: 15% of CHUNK_SIZE)
...
- `dyslexxy.domain.notebook`: Source, Note, SourceInsight models; vector_search function
- `dyslexxy.ai.models`: model_manager for embedding model access
- `dyslexxy.exceptions`: DatabaseOperationError, NotFoundError
...
- **Chunk size semantics changed**: `DYSLEXXY_CHUNK_SIZE` and `DYSLEXXY_CHUNK_OVERLAP` are token-based, not character-based
...
3. **Change chunk size**: Set DYSLEXXY_CHUNK_SIZE and DYSLEXXY_CHUNK_OVERLAP environment variables
...
### Chunking
```python
from dyslexxy.utils.chunking import chunk_text, detect_content_type, ContentType
...
### Embedding
```python
from dyslexxy.utils.embedding import generate_embedding, generate_embeddings
...
### Context Building
```python
from dyslexxy.utils.context_builder import ContextBuilder, ContextConfig
...
**Key behavior**:
- Key source: DYSLEXXY_ENCRYPTION_KEY_FILE (Docker secrets) → DYSLEXXY_ENCRYPTION_KEY (env var)
...
- DYSLEXXY_ENCRYPTION_KEY must be set explicitly (no default)
...
**Usage Example**:
```python
from dyslexxy.utils.encryption import encrypt_value, decrypt_value
