"""
Chunking utilities for Dyslexxy.
...
def _get_chunk_size() -> int:
    ...
    chunk_size_str = os.getenv("DYSLEXXY_CHUNK_SIZE")
    if chunk_size_str:
        try:
            chunk_size = int(chunk_size_str)
            if chunk_size < 100:
                logger.warning(
                    f"DYSLEXXY_CHUNK_SIZE ({chunk_size}) is too small. "
                    f"Using minimum value of 100."
                )
                return 100
            if chunk_size > 8192:
                logger.warning(
                    f"DYSLEXXY_CHUNK_SIZE ({chunk_size}) is very large. "
            ...
            logger.warning(
                f"Invalid DYSLEXXY_CHUNK_SIZE value: '{chunk_size_str}'. "
                f"Using default: 400"
            )
    return 400
...
def _get_chunk_overlap(chunk_size: int) -> int:
    ...
    overlap_str = os.getenv("DYSLEXXY_CHUNK_OVERLAP")
    if overlap_str:
        try:
            overlap = int(overlap_str)
            if overlap < 0:
                logger.warning(
                    f"DYSLEXXY_CHUNK_OVERLAP ({overlap}) cannot be negative. "
                    f"Using 0."
                )
                return 0
            if overlap >= chunk_size:
                logger.warning(
                    f"DYSLEXXY_CHUNK_OVERLAP ({overlap}) cannot be >= chunk size ({chunk_size}). "
            ...
            logger.warning(
                f"Invalid DYSLEXXY_CHUNK_OVERLAP value: '{overlap_str}'. "
