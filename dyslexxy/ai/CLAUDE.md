...
- Singleton configuration record (record_id: `dyslexxy:default_models`)
...
- `dyslexxy.database.repository`: repo_query, ensure_record_id
- `dyslexxy.domain.base`: ObjectModel, RecordModel base classes
- `dyslexxy.domain.credential`: Credential for database-stored API keys
- `dyslexxy.utils`: token_count() for context size detection
