...
- `dyslexxy.ai.provision`: `provision_langchain_model()` (async factory with fallback logic)
- `dyslexxy.utils.error_classifier`: `classify_error()` for user-friendly LLM error messages
- `dyslexxy.domain.notebook`: Domain models (Source, Note, SourceInsight, vector_search)
- `loguru`: Logging
