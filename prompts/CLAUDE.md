...
Jinja2 prompt templates for multi-provider AI workflows in Dyslexxy.
...
See detailed workflow integration in `dyslexxy/graphs/CLAUDE.md` for how each template fits into chat.py, ask.py, source_chat.py.
...
See `dyslexxy/graphs/ask.py` for how entry.jinja is invoked inside ask_graph workflow.

## Reference Documentation

- **Jinja2 syntax guide**: See existing templates for for-loop, if-conditional, variable interpolation patterns
- **Graph integration**: `dyslexxy/graphs/CLAUDE.md` documents which template is used in which workflow
